...
""")
    ...
    def _update_status_worker(self):
        ...
        try:
            ...
            # Browser status (just file existence check)
            ff_active = os.path.exists(self.ff_desktop)
            ch_active = os.path.exists(self.ch_desktop)

            # Update UI on main thread
            self.after(0, lambda: self._update_ui(osk_active, ks_status, ff_active, ch_active))
        except Exception as e:
            print(f"Kiosk update error: {e}")
        finally:
            self.is_updating = False

    def _update_ui(self, osk_active, ks_status, ff_active, ch_active):
        ...
        # Killswitch Button
        if ks_status == "running": self.btn_toggle_ks.configure(text=txt("btn_ks_on"), fg_color=self.colors["success"])
        elif ks_status == "stopped": self.btn_toggle_ks.configure(text=txt("btn_ks_installed"), fg_color=self.colors["danger"])
