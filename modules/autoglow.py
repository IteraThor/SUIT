...
class AutoGlowView(ctk.CTkFrame):
    ...
    def _run_cmd(self, action):
        script = ServiceUtils.sudo_cmd(f"systemctl {action} {self.SERVICE_NAME}")
        ServiceUtils.run_bash_script(self, script, f"AutoGlow {action}", on_close=self.update_status)

    def update_status(self):
        """Triggers a non-blocking update of service and hardware status."""
    ...
    def _update_status_worker(self):
        ...
        try:
            ...
            ws_conn = self.check_ws_connection()
            
            # Update UI on main thread
            self.after(0, lambda: self._update_ui(status, esp_port, ws_conn))
        except Exception as e:
            print(f"Update error: {e}")
        finally:
            self.is_updating = False

    def _update_ui(self, status, esp_port, ws_conn):
