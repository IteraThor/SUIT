...
# FIX: Eigentümerschaft auf den aktuellen User ändern
chown -R "{current_user}":"{current_user}" AutoGlow

cd AutoGlow
echo "Starte Setup..."

# Export SUDO_USER für setup.sh
...
./setup.sh
    ...
    def open_config_gui(self):
        ...
        # FIX: Display-Variablen für Root verfügbar machen
        display = os.environ.get("DISPLAY", ":0")
        xauth = os.environ.get("XAUTHORITY", "")
        
        # Erlaubt root den Zugriff auf das Display (wichtig für XWayland/X11)
        try:
        ...
        cmd = []
        if which("pkexec"):
            cmd = ["pkexec", "env", f"DISPLAY={display}", f"XAUTHORITY={xauth}", venv_python, gui_script]
        else:
            # Fallback sudo
            cmd = ["sudo", "env", f"DISPLAY={display}", f"XAUTHORITY={xauth}", venv_python, gui_script]

        try:
            # HIER IST DER FIX: Arbeitsverzeichnis (cwd) setzen!
            subprocess.Popen(cmd, cwd=self.autoglow_dir)
        except Exception as e:
            messagebox.showerror("Error", f"Failed to open GUI: {e}")
